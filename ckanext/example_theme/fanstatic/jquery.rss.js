(function($) {
  ...
  RSS.prototype.load = function(callback) {
    var apiProtocol = "https"
      , apiHost     = apiProtocol + "://api.rss2json.com/v1/api.json"
      , apiUrl      = apiHost + "?rss_url=" + encodeURIComponent(this.url) + "&order_by=pubDate&order_dir=desc"

    if (this.options.limit != null) apiUrl += "&count=" + this.options.limit;
    if (this.options.key != null)   apiUrl += "&api_key=" + this.options.key;
  ...
  RSS.prototype.getTokenMap = function(entry) {
    ...
    return $.extend({
      ...
      shortBodyPlain: (function(entry) {
       //AZ : inserire continua a leggere
       //AZ: parametrizzare feed
       //entry.description.replace(/<\/?[^>]+>/gi, ''),
       var content = entry.content
       //var clean_start= content.indexOf("</h3>")
       //if(clean_start !== -1) content = content.substr(clean_start+5)
       content = content
         .replace(/<script[\\r\\\s\S]*<\/script>/mgi, '')
        //  .replace(/<\/?[^>]+>/gi, '')

       //var result = entry.description.replace(/<\/?[^>]+>/gi, '')
       var result = content.substr(0,800)+ "...   <a  href=\""+ entry.link + "\">"+ " continua a leggere"+ "</a>"
        //for(var i = 0; i < RSS.htmlTags.length; i++) {
        //  result = result.replace(new RegExp('<' + RSS.htmlTags[i], 'gi'), '')
        //}
